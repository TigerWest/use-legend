...
Before implementing any component, read and apply these 4 references. Keep them in active working context for the entire task.
...
- `references/component-props.md` -- `DeepMaybeObservable<Props>`, pass-through, children, callbacks
...
- `.get()` only inline in JSX or inside reactive contexts (`observe`, `observable(() => ...)`)
...
- `children` treated as normal React prop, never as observable
