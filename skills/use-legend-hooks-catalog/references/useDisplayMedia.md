...
<CodeTabs>
  ...
  <Fragment slot="scope">
    ```tsx
    import { createDisplayMedia } from "@usels/web";

    function ScreenShare() {
      "use scope";
      const { isSupported$, stream$, enabled$, start, stop } = createDisplayMedia();
...
- Documentation: `packages/web/src/sensors/useDisplayMedia/index.mdx`