...
const OUT_DIR = join(ROOT, 'skills/use-legend-hooks-catalog')
...
function discoverHooks(srcDir: string, pkgName: string): HookInfo[] {
  ...
  for (const cat of readdirSync(srcDir, { withFileTypes: true })) {
    ...
    for (const hook of readdirSync(catDir, { withFileTypes: true })) {
      ...
      const mdxPath = join(hookDir, 'index.mdx')
      const mdPath = join(hookDir, 'index.md')
      const tsPath = join(hookDir, 'index.ts')
      const docPath = existsSync(mdxPath)
        ? mdxPath
        : existsSync(mdPath)
          ? mdPath
          : null
      if (!docPath) continue
      const docExt: 'mdx' | 'md' = docPath === mdxPath ? 'mdx' : 'md'
...
function generateReference(hook: HookInfo): string {
  ...
  parts.push(
    ...
    `- Documentation: \`${hook.relativeSource}/index.${hook.docExt}\``,
  )
...
// ----- SKILL.md generation -----
function generateSkillMd(hooks: HookInfo[]): string {
    ...
    for (const cat of Object.keys(byPkg[pkg]).sort()) {
      ...
      for (const h of catHooks) {
        const desc = h.description.length > 100
          ...
          : h.description
        lines.push(`| [\`${h.name}\`](references/${h.name}.md) | ${desc} |`)
      }
      lines.push('')
    }
  }
...
for (const hook of allHooks) {
  if (writtenNames.has(hook.name)) {
    throw new Error(
      `Duplicate hook name "${hook.name}" — two hooks would write to the same reference file. Check frontmatter titles in source index.mdx files.`,
    )
  }
  writtenNames.add(hook.name)
...
writeFileSync(join(OUT_DIR, 'SKILL.md'), generateSkillMd(allHooks), 'utf-8')
