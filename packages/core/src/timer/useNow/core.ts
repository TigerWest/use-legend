 ...
 */
export function now(options?: NowOptions): Disposable & Pausable & { now$: Observable<Date> } {
  ...
  const scheduler = isRaf
    ? rafFn(update, { immediate })
    : intervalFn(update, observable(typeof interval === "number" ? interval : 1000), { immediate });
