...
export function addAutoImport(
  ...
  const alreadyImported = programPath.node.body.some((node) => {
    ...
    return node.specifiers.some(
      (s) => t.isImportSpecifier(s) && t.isIdentifier(s.imported, { name: autoComponentName })
    );
  });
  ...
  const importDecl = t.importDeclaration(
    [t.importSpecifier(t.identifier(autoComponentName), t.identifier(autoComponentName))],
