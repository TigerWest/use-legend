...
**`options` is `DeepMaybeObservable`.** Each option field can be a plain value or an `Observable`. `constraints` are read at each `start()` / `restart()` call time. `immediate` is mount-time-only.
