...
```tsx
import { observable, createObserve, toObs } from "@usels/core";
...
`props$.value.get()` is reactive and re-runs the `createObserve()` callback when the
...
```tsx
import { observable, createObserve, toObs, useScope } from "@usels/core";
...
For reactive side effects inside a scope, see [Effects API](/use-legend/guides/use-scope/effects-api/) — the canonical reference for `createObserve()`, `watch()`, and `whenever()`.
