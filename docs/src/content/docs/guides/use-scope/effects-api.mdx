---
title: Effects API
description: Canonical reference for createObserve, watch, and whenever — the three scope-level effect primitives.
---
...
```ts
function createObserve(callback: () => void): () => void;
```

Returns a dispose function. Inside a scope the dispose is auto-registered — you do not need to capture the return value.

> The legacy export name `observe` is preserved as a deprecated alias for backwards compatibility.

**Example:**

```tsx
import { createObserve, observable } from "@usels/core";
...
| Need | API |
| --- | --- |
| React to any observable read inside the effect body (fires on mount) | `createObserve()` |
